...
Next planned version: `v1.3.0`

This document defines the implementation path from `v1.2.0` forward.
Completed work for earlier releases is retained below as release history.
...
## v1.0.2: QC and Gap-Fill Hardening (Completed)
...
## v1.1.0: Timezone-Safe Ingest and Export (Completed)
...
## v1.2.0: Weathercloud Ingest and Model Features (Completed)

Status: completed in the current codebase and covered by regression tests.

### Goal

Support dissertation Weathercloud ingestion needs directly in MetDataPy and add reusable model-facing meteorological feature helpers.

### Task 9: Add Weathercloud multi-file ingestion and delimiter support

**Primary files**
- `metdatapy/io.py`
- `metdatapy/weathercloud.py`
- `tests/test_encoding.py`
- `tests/test_weathercloud.py`

**Implementation**
- Add delimiter detection to the shared CSV reader while preserving encoding detection.
- Add `read_weathercloud_csv(path, mapping_config=None, timezone="Europe/Athens", delimiter=None)`.
- Add `read_weathercloud_directory(path, mapping_config, timezone="Europe/Athens", delimiter=None)`.
- Return raw source columns when no mapping is supplied to `read_weathercloud_csv`.
- Return canonical, UTC-indexed, unit-normalized data when a mapping is supplied.

**Acceptance criteria**
- Semicolon-delimited Weathercloud exports load correctly.
- A directory of Weathercloud CSV exports concatenates into one UTC-indexed canonical dataframe.
- Empty or non-directory Weathercloud inputs fail with clear errors.

### Task 10: Add rain-rate canonical schema support
...
**Implementation**
- Add `rain_rate_mmh` to canonical fields, plausible bounds, mapper detection, unit normalization, default resampling, and NetCDF metadata.
- Support `mm/h` and `inch/h` unit hints.
- Keep rain-rate mapping distinct from accumulated/event rainfall.

**Acceptance criteria**
- Weathercloud rain-rate columns map to `rain_rate_mmh`, not `rain_mm`.
- Rain-rate values normalize to `mm/h`.
- QC range flags and NetCDF metadata include `rain_rate_mmh`.

### Task 11: Add wind direction encoding and rolling features
...
**Implementation**
- Add `WeatherSet.encode_wind_direction(drop_original=False)`.
- Add `WeatherSet.rolling_features(columns, windows, stats=("mean", "std", "min", "max"), closed="left")`.
- Use past-only rolling windows by default to avoid leakage.

**Acceptance criteria**
- `wdir_deg` produces `wdir_sin` and `wdir_cos` and is preserved by default.
- Rolling features exclude the current row when `closed="left"`.
- Invalid rolling stats and windows fail with clear errors.

## v1.3.0: Meteorological Correctness

### Goal

Fix domain-invalid derived metrics and physically incorrect aggregation behavior.

### Task 12: Make derived thermal indices domain-aware
...
### Task 13: Fix consistency QC and wind-direction resampling semantics
...
## v1.4.0: Provenance-Preserving Imputation

### Goal

Add missing-data handling without losing auditability.

### Task 14: Add a dedicated imputation module
...
### Task 15: Integrate imputation into reproducibility and CLI
...
### Task 17: Implement holiday features
...
### Task 19: Add rolling-origin backtesting to ML prep
...
3. Ship `v1.2.0` to support Weathercloud ingestion and reusable model feature preparation.
4. Ship `v1.3.0` before broader feature work so derived metrics and resampling are physically correct.
5. Ship `v1.4.0` and `v1.5.0` next as the most natural product extensions.
6. Ship `v1.6.0` last after the primitives are corrected and stable.
...
- `v1.2.0`
  - Weathercloud delimiter and directory ingestion
  - canonical `rain_rate_mmh`
